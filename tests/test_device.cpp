...
int main() {
  ...
  err = clGetDeviceInfo(device, CL_DEVICE_MAX_COMPUTE_UNITS, sizeof(compute_units), &compute_units,
                        NULL);
  if (err != CL_SUCCESS) {
  ...
  size_t sz = 0;
  err = clGetDeviceInfo(device, CL_DEVICE_PLATFORM, sizeof(queried), &queried, &sz);
