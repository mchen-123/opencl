...
int main(void)
{
    ...
    cl_device_partition_property props[] = {
        CL_DEVICE_PARTITION_BY_DIE_LIST,
        2, 3,      // die2 和 die3
        0
    };
    ...
    for (int i = 0; i < 2; i++) {
        ...
        clGetDeviceInfo(sub_devs[i], CL_DEVICE_MAX_COMPUTE_UNITS, 
                        sizeof(compute_units), &compute_units, NULL);
        clGetDeviceInfo(sub_devs[i], CL_DEVICE_VENDOR_ID, 
                        sizeof(vendor_id), &vendor_id, NULL);

        printf("  Sub Device %d: CU=%u, VendorID=0x%04x\n", i, compute_units, vendor_id);
    }
