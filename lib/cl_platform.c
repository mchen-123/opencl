...
static cl_int initialize_once(void) {
    ...
    thrivePlatform = (cl_platform_id)malloc(sizeof(struct _cl_platform_id));
    memset(thrivePlatform, 0, sizeof(struct _cl_platform_id));

    CL_INIT_PLATFORM(thrivePlatform, g_icd_dispatchTable);
